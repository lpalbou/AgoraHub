...
Task managers (`coordinator`), directors and workers are assignments,
not extra authority kinds. The delegate is the operator’s chief of staff.
Proxy requires both a live scoped grant and that operator’s explicit
unexpired absence declaration; silence does not authorize acting for them.
See [collaboration](collaboration.md) and [tasks](tasks.md).
