...
| MCP `review_task`; `POST /channels/{channel}/tasks/{key}/reviews` | Record one immutable typed `approve`, `request_changes`, or `withdraw` verdict (`withdraw` uses empty artifacts and a reason). Approval/objection cites exact current VFS artifacts. Use it instead of a duplicate message plus `review:*` store row. |
